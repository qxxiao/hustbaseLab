...
void CHustBaseApp::OnCreateDB()
{
	...
	openFileDlg.m_ofn.lpstrTitle = "创建数据库";
	openFileDlg.m_ofn.lpstrInitialDir="C:";	//默认打开的文件路径
	INT_PTR result = openFileDlg.DoModal();
	CString dbPath, dbName;
	if (result == IDOK) {
		//dbPath = openFileDlg.GetFolderPath();//添加默认路径后不起作用
		dbPath = openFileDlg.GetPathName();
		dbPath = dbPath.Left(dbPath.ReverseFind('\\'));
		dbName = openFileDlg.GetFileName();
	}
	else 
		return;
	CreateDB((LPTSTR)(LPCTSTR)dbPath, (LPTSTR)(LPCTSTR)dbName);
}

void CHustBaseApp::OnOpenDB() 
{
	//关联打开数据库按钮，此处应提示用户输入数据库所在位置，并调用OpenDB函数改变当前数据库路径，并在界面左侧的控件中显示数据库中的表、列信息。
	CFolderPickerDialog fpDlg("C:", 0, NULL, 0);
	fpDlg.m_ofn.lpstrTitle = "打开数据库";
	INT_PTR result = fpDlg.DoModal();
	CString path;
	if (result == IDOK) {
		path = fpDlg.GetPathName();
	}
	else
		return;
	///////////////////////
	TCHAR buffer[100];
	LPTSTR lpszCurDir = buffer;
	GetCurrentDirectory(100, lpszCurDir);
	CString curPath = lpszCurDir;
	if (CHustBaseApp::pathvalue && path == curPath)
		return;
	if (OpenDB((LPTSTR)(LPCTSTR)path) == SUCCESS)
	{
...
void CHustBaseApp::OnDropDb() 
{
	...
	if (result == IDOK) {
		...
		return;
	if (DropDB((LPTSTR)(LPCTSTR)path) == SUCCESS)
	{
		CFileFind finder;
		bool isExist = finder.FindFile(path + "\\*.*");
		finder.Close();
		if (isExist && CHustBaseApp::pathvalue)
		{
			SetCurrentDirectory(path + "\\..");
			...
			CHustBaseDoc* pDoc;
			pDoc = CHustBaseDoc::GetDoc();
			pDoc->m_pTreeView->GetTreeCtrl().DeleteAllItems();
			pDoc->m_pListView->ClearList();
			pDoc->m_pEditView->GetEditCtrl().Clear();
		}
		else
			RemoveDirectory(path);
	}
