...
int CEditArea::iReadDictstruct(char tabname[][20],int *tabnum,char colname[][20][20],int colnum[],AttrType coltype[][20],int collength[][20],int coloffset[][20],int iscolindex[][20])//编写新的传递系统表和系统列信息的函数
{
	...
	CString table=Path+"\\SYSTABLES";		//不加后缀
	CString column=Path+"\\SYSCOLUMNS";		//不加后缀
	...
	/*rec1.pData = (char*)malloc(fileHandle.fileSubHeader->recordSize);
	rec2.pData = (char*)malloc(colfilehandle.fileSubHeader->recordSize);*/
